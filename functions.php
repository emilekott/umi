...
function add_plugin( $plugin_array ) {
   $plugin_array['vimeo'] = get_template_directory_uri() . '/js/vimeo.js';
   return $plugin_array;
}
...
function ttrust_scripts() {
	...
	wp_enqueue_script('slideshow', get_bloginfo('template_url').'/js/jquery.flexslider-min.js', array('jquery'), '1.0', true);	
...
<?php if($ttrust_color_scheme) : ?>
	#header {background-color: <?php echo($ttrust_color_scheme); ?>;}
	#mainNav ul ul {background-color: <?php echo($ttrust_color_scheme); ?>;}
<?php endif; ?>
